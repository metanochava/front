          ...
          <div class="col-12 col-sm-6 col-md-4">
            <div class="vitals-context">
              ...
              <div class="q-ml-md ellipsis">
                ...
                <div v-if="!ctx.agenda" class="text-body2 text-weight-medium" data-test="vital-signs-no-appointment">
                  {{ tdc('No appointment today') }}
                </div>
                <div v-else class="text-body2 text-weight-medium">{{ ctx.agenda.date }} {{ ctx.agenda.time || '' }}</div>
                <div v-if="ctx.agenda" class="text-caption ellipsis">
...
const props = defineProps({
  ...
  // without an appointment row (the patient header): the patient; the server
  // uses today's open appointment of this patient when there is one
...
async function load() {
  ...
  loading.value = true
  try {
    const { data } = await HTTPAuth.get(url({ type: 'u', url: 'saude/dadovitals/intake/', params }))
    ctx.value = data
...
watch(() => [props.modelValue, props.context?.id, props.patientId], ([isOpen]) => { if (isOpen) load() }, { immediate: true })
